...
def update_doc(infnm, outfnm, mark_miles=True):
    """
    ...
    indoc = fix_milestones(indoc)
    outdoc = get_template_doc(outfnm, True)
    pstyl = get_style(outdoc)
    found_body = False
    for ind, p in enumerate(indoc.paragraphs):
        if not found_body:
            if "0 Body" in p.style.name:
                found_body = True
            continue
...
def fix_milestones(adoc):
    '''
    If line milestones do not have page number in them
    :param adoc:
    :return:
    '''
    last_page = False
    for p in adoc.paragraphs:
        for rn in p.runs:
            rsty = rn.style.name
            if "Page Number Print" in rsty:
                last_page = rn.text.strip('[]')
            if "Line Number Print" in rsty:
                rtxt = rn.text.strip('[]')
                if '.' not in rtxt:
                    rn.text = '[{}.{}]'.format(last_page, rtxt)
    return adoc
...
def convert_files(ind, outd, milestones):
    ct = 0
    for fnm in [f for f in listdir(ind) if f.endswith('.docx')]:
        ...
        update_doc(inf, outf, milestones)
        ct += 1

    print("{} documents done!".format(ct))


parser = argparse.ArgumentParser(description='Copy old Word docs to template with new styles and table')
...
if __name__ == "__main__":
    ...
    convert_files(indir, outdir, kwargs['milestones'])
